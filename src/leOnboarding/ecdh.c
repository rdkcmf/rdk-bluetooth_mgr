  ...
  int* clear_text_length)
{
  ...
  if (!private_key)
  {
    BTRMGRLOG_ERROR("failed to read private key from '%s'\n", private_key_path);
    ret = 0;
    goto error;
  }
  ...
  if (!peer_key)
  {
    BTRMGRLOG_ERROR("failed to read public key from pem data\n");
    BTRMGRLOG_INFO(" --------- PEER PUBLIC KEY -----------\n%s\n",
      peer_public_key);
    ret = 0;
    goto error;
  }
  ...
  if (!ret)
  {
    BTRMGRLOG_ERROR("failed to derive shared key\n");
    goto error;
  }
  ...
  if (!ret)
  {
    BTRMGRLOG_ERROR("failed to decrypt data\n");
    goto error;
  }
...
baFromBase64(byteArray_t* decoded_data, char const* encoded_data)
{
  ...
  if (!decoded_data)
  {
    BTRMGRLOG_ERROR("no parameter sepcified to base64 decode into\n");
    return 0;
  }

  if (!encoded_data)
  {
    BTRMGRLOG_ERROR("no parameter specified to base64 decode\n");
    return 0;
  }
  ...
  byteArray_t*       plain_data)
{
  ...
  if (!cipher_ctx)
  {
    BTRMGRLOG_ERROR("EVP_CIPHER_CTX_new failed. %s\n", CRYPTO_Error());
    baClear(plain_data);
    return 0;
  }
  ...
  if (!ret)
  {
    BTRMGRLOG_ERROR("EVP_DecryptInit_ex failed. %s\n", CRYPTO_Error());
  ...
  {
    BTRMGRLOG_ERROR("EVP_DecryptUpdate failed. %s\n", CRYPTO_Error());
    EVP_CIPHER_CTX_free(cipher_ctx);
    return 0;
  }
  ...
  {
    BTRMGRLOG_ERROR("EVP_DecryptFinal_ex failed. %s\n", CRYPTO_Error());
    EVP_CIPHER_CTX_free(cipher_ctx);
    return 0;
  }
  ...
  char const* pem_data)
{
  ...
  if (!public_key)
  {
    BTRMGRLOG_ERROR("failed to parse PEM data for public key. %s\n", CRYPTO_Error());
  }
  BIO_free_all(buff);
  ...
  char const* fname)
{
  ...
  if (!fname)
  {
    BTRMGRLOG_ERROR("can't read public key from NULL filename\n");
    return NULL;
  }
  ...
  {
    int err = errno;
    BTRMGRLOG_ERROR("failed to open file to read public key '%s'. %d\n",
      fname, err);
    return NULL;
  }
...
ECC_ReadPrivateKeyFromFile(char const* fname)
{
  ...
  {
    int err = errno;
    BTRMGRLOG_ERROR("failed to open file to read private key '%s'. %d\n",
      fname, err);
    return NULL;
  }
  ...
  byteArray_t* shared_secret)
{
  ...
  if (!key_ctx)
  {
    ...
    BTRMGRLOG_ERROR("EVP_PKEY_CTX_new failed:%lu", err);
    return 0;
  }
  
  if (!EVP_PKEY_derive_init(key_ctx))
  {
    BTRMGRLOG_ERROR("EVP_PKEY_derive_init failed. %s", CRYPTO_Error());
  ...
  {
    BTRMGRLOG_ERROR("EVP_PKEY_derive_set_peer failed. %s", CRYPTO_Error());
    EVP_PKEY_CTX_free(key_ctx);
    return 0;
  }
  ...
  {
    BTRMGRLOG_ERROR("EVP_PKEY_derive failed. %s", CRYPTO_Error());
    EVP_PKEY_CTX_free(key_ctx);
    return 0;
  }
  ...
  cJSON** wifi_settings)
{
  ...
  if (!server_reply)
  {
    BTRMGRLOG_ERROR("null server_reply\n");
    return 1;
  }

  if (!wifi_settings)
  {
    BTRMGRLOG_ERROR("null wifi_settings\n");
    return 1;
  }
  ...
  if (settings)
  {
    BTRMGRLOG_INFO("found un-encrypted settings, using those\n");
  ...
  if (!settings)
  {
    BTRMGRLOG_ERROR("failed to find 'secure-wifi-settings' in wifi settings object\n");
    *wifi_settings = NULL;
    return 0;
  }
  ...
  if (!iv)
  {
    BTRMGRLOG_ERROR("failed to find 'iv' in wifi settings object\n");
    *wifi_settings = NULL;
    return 0;
  }
  ...
  if (!peer_key)
  {
    BTRMGRLOG_ERROR("failed to find 'pubKey' in wifi settings object\n");
    return 0;
  }
  ...
  if (!ret)
  {
    BTRMGRLOG_ERROR("failed to decrypt secure wifi settings from '%s'",
  ...
  if (!clear_text)
  {
    BTRMGRLOG_ERROR("encryption returned NULL clear text string\n");
    return 0;
  }
  ...
  *wifi_settings = cJSON_Parse(clear_text);
  if (!*wifi_settings)
  {
    BTRMGRLOG_ERROR("failed to parse secure wifi settings json string '%s'",
      clear_text);
    return 0;
  }
